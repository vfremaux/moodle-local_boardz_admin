...
define(['jquery', 'core/log', 'core/config'], function($, log, cfg) {
    ...
    var boardzadminpro = {
        ...
        init: function() {
            ...
            $('#id_s_local_boardz_admin_licensekey').trigger('change');
            log.debug('AMD Pro js initialized for local boardz admin');
        },
        ...
        check_product_key: function() {
            ...
            var calculated = boardzadminpro.checksum(payload);
            ...
            if (crc === calculated) {
                ...
                url += '&provider=' + $('#id_s_local_boardz_admin_licenseprovider').val();

                $('#id_s_local_boardz_admin_licensekey + img').remove();
                $('#id_s_local_boardz_admin_licensekey').after(waiticon);

                $.get(url, function(data) {
                    if (data.match(/SET OK/)) {
                        $('#id_s_local_boardz_admin_licensekey + img').remove();
                        $('#id_s_local_boardz_admin_licensekey').after(validicon);
                    } else {
                        $('#id_s_local_boardz_admin_licensekey + img').remove();
                        $('#id_s_local_boardz_admin_licensekey').after(invalidicon);
                    }
                }, 'html');
            } else {
                $('#id_s_local_boardz_admin_licensekey + img').remove();
                $('#id_s_local_boardz_admin_licensekey').after(cautionicon);
            }
        },
